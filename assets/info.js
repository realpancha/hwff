...
async function checkUsAdmin(PANCHIMSG, user = PANCHIMSG.data.participant) {
...
PANCHI.operate({ pattern: 'info', fromMe: Work_Mode, desc: Lang.INFO_DESC, deleteCommand: false}, async (PANCHIMSG) => { 
        
    if (PANCHIMSG.jid.includes('g.us')) {

        var im = await checkImAdmin(PANCHIMSG)
        if (!im) {
            var invite = Lang.GRP_COD + '\n' + Lang.NOADMIN
        } else {
            const code = await PANCHIMSG.client.groupInviteCode(PANCHIMSG.jid)
            var invite = Lang.GRP_COD + `\n https://chat.whatsapp.com/${code}`
        }

        var json = await PANCHIMSG.client.groupMetadataMinimal(PANCHIMSG.jid) 

        var nwjson = await PANCHIMSG.client.groupMetadata(PANCHIMSG.jid) 

        const msg = Lang.GRP_NAME + `\n ${nwjson.subject} \n\n` + Lang.GRP_JID + `\n ${json.id} \n\n` + Lang.GRP_OWN + `\n ${json.owner} \n\n` + invite +`\n\n` + Lang.GRP_DES + `\n ${nwjson.desc}`

        var ppUrl = await PANCHIMSG.client.getProfilePicture(PANCHIMSG.jid) 

        const resim = await axios.get(ppUrl, {responseType: 'arraybuffer'})
        const media = Buffer.from(resim.data)

        await PANCHIMSG.sendMessage(
            media, 
            MessageType.image, 
        ...
        var status = await PANCHIMSG.client.getStatus(PANCHIMSG.jid) 
        var usppUrl = await PANCHIMSG.client.getProfilePicture(PANCHIMSG.jid) 
        var usexists = await PANCHIMSG.client.isOnWhatsApp(PANCHIMSG.jid)
...
PANCHI.operate({ pattern: 'qawelinfo', fromMe: false, deleteCommand: false, dontAddCommandList: true}, async (PANCHIMSG) => { 
...
PANCHI.operate({pattern: 'covid ?(.*)', fromMe: Work_Mode, desc: LANG.COVID_DESC,  deleteCommand: false}, async (PANCHIMSG, match) => {
    if (match[1] === '') return await PANCHIMSG.reply(LANG.NEED_CON);
        const url = `https://coronavirus-19-api.herokuapp.com/countries/${match[1]}`;
        const response = await got(url);
        const jsun = JSON.parse(response.body);
        await PANCHIMSG.client.sendMessage(PANCHIMSG.jid, fs.readFileSync("./node_modules/PANCHI-public/media/gif/earth.mp4"), MessageType.video, {mimetype: Mimetype.gif, quoted: PANCHIMSG.data, caption: LANG.COUNTRY + jsun.country + '\n\n' + LANG.CASES + jsun.cases + '\n' + LANG.TCASES + jsun.todayCases + '\n' + LANG.DEATHS + jsun.deaths + '\n' + LANG.TDEATHS + jsun.todayDeaths + '\n' + LANG.RECO + jsun.recovered + '\n' + LANG.CRIT + jsun.critical + '\n' + LANG.TEST + jsun.totalTests, thumbnail: qathmub });
});
