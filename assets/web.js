...
PANCHI.operate({pattern: 'speedtest ?(.*)', fromMe: true, desc: Lang.SPEEDTEST_DESC}, (async (message, match) => {
    await PANCHI.PANCHI_setup()
    if (match[1] == 'user' || match[1] == 'User') {
        
        // Preliminary Message
        var transition_message = await PANCHI.speedtest_begin(Build.LANG)
        ...
        // Speedtest Modules
        var user_download = await PANCHI.speedtest_user()
        var user_upload = await PANCHI.speedtest_userup()
        var auth_message = await PANCHI.speedtest_message(Build.LANG)
        ...
        // Speedtest Modules
        var server_download = await PANCHI.speedtest_server()
        var server_upload = await PANCHI.speedtest_serverup()
        var auth_message = await PANCHI.speedtest_message(Build.LANG)
        var act_ping = end - start
        ...
        // Preliminary Message
        var transition_message = await PANCHI.speedtest_once(Build.LANG)
...
PANCHI.operate({pattern: 'ping', fromMe: true, deleteCommand: false, desc: Lang.PING_DESC, dontAddCommandList: true}, (async (message, match) => {
  var start = new Date().getTime();
...
PANCHI.operate({pattern: 'short ?(.*)', fromMe: false, desc: Lang.URL}, (async (message, match) => {
...
PANCHI.operate({pattern: 'getqr ?(.*)', fromMe: true, desc: Lang.AMDI_QR,  deleteCommand: false}, (async (PANCHIMSG, input) => {
    await PANCHI.PANCHI_setup()
    const CMD = input[1]
    if (PANCHIMSG.jid.includes('g.us')) {
        await PANCHIMSG.client.sendMessage(PANCHIMSG.jid, Lang.INBOX, MessageType.text, {quoted: PANCHIMSG.data});
    } else {
        await _PANCHI.PANCHIQR( PANCHIMSG, CMD, Lang )
    }
}));
