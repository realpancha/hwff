...
PANCHI.operate({pattern: 'sticker$', fromMe: Work_Mode, desc: Lang.STICKER_DESC,  deleteCommand: false}, (async (message, match) => {    
...
PANCHI.operate({pattern: 'sticvid$', fromMe: Work_Mode, desc: Lang.ANI_STICK,  deleteCommand: false}, (async (message, match) => {
    await PANCHI.amdi_setup()
    ...
    await webp2mp4File(savedFilename).then(async (rest) => {
        await axios({ method: "GET", url: rest.result, responseType: "stream"}).then(({ data }) => {
            const saving = data.pipe(fs.createWriteStream('/root/PANCHI/vstic.mp4')) //OI OI MEKA WENAS KRAN ROOT
            saving.on("finish", async () => {
                await message.client.sendMessage(msgid, fs.readFileSync('/root/PANCHI/vstic.mp4'), MessageType.video, { mimetype: Mimetype.mp4, caption: Build.CAP, quoted: message.data , thumbnail: qathmub })
                if (fs.existsSync(savedFilename)) fs.unlinkSync(savedFilename)
                if (fs.existsSync('/root/PANCHI/vstic.mp4')) fs.unlinkSync('/root/PANCHI/vstic.mp4')
            })
        })
    })
    return await message.client.deleteMessage(message.jid, {id: downloading.key.id, remoteJid: message.jid, fromMe: true})
